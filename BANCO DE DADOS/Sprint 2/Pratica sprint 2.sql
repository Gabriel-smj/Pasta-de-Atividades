...
UPDATE pessoa SET cpf = NULL WHERE idPessoa = 4;
UPDATE reserva SET dtReserva = NULL, dtRetirada = NULL, dtDevolucao = NULL WHERE idReserva = 4;

SELECT pessoa.idPessoa AS Identificador,
nome AS NOME, 
IFNULL(cpf, 'Não informado') AS CPF,
IFNULL(dtReserva, 'Sem Reserva') AS Reserva,
IFNULL(dtRetirada, 'Sem Retirada') AS Retirada,
IFNULL(dtDevolucao, ' ') AS Devolução
FROM reserva JOIN pessoa
ON idPessoa = fkPessoa;
...
IFNULL(cpf, 'Não informado') AS CPF,
CASE
WHEN nome = 'Nathan' THEN 'Davi'
WHEN nome = 'David' THEN 'Matheus'
WHEN nome = 'Fernanda' THEN 'Fernando'
WHEN nome = 'Vitoria' THEN 'Victoria'
WHEN nome = 'Luan' THEN 'Victor'
END AS NOME,
IFNULL(dtReserva, 'Sem Reserva') AS Reserva,
IFNULL(dtRetirada, 'Sem Retirada') AS Retirada,
IFNULL(dtDevolucao, ' ') AS Devolução
...
UPDATE pessoa2 JOIN pessoa1 ON idPessoa1 = fkPessoa1 
SET pessoa1.dtNascimento = Null, pessoa2.dtNascimento = NULL WHERE pessoa1.idPessoa1 = 1 AND pessoa2.idPessoa2 = 1;

SELECT pessoa1.idPessoa1 AS IDENTIFICADOR_1,
pessoa1.nome AS NOME,
IFNULL(pessoa1.dtNascimento, 'Não Informado') AS DataNascimento_1,
pessoa2.idPessoa2 AS IDENTIFICADO_2,
pessoa2.nome AS NOME,
IFNULL(pessoa2.dtNascimento, 'Não Informado') AS DataNascimento_2
FROM pessoa2 JOIN pessoa1 ON idPessoa1 = fkPessoa1;
...
UPDATE usuario SET cpf = NULL, dtNascimento = NULL WHERE idUsuario = 2;

SELECT nome AS NOMES,
IFNULL(cpf, 'Não Informado') AS CPF,
IFNULL(dtNascimento, 'Não Informado') AS DataNascimento
FROM usuario;
...
ALTER TABLE Habilitacao MODIFY COLUMN validade VARCHAR(20);
UPDATE Habilitacao SET validade = NULL WHERE idHabilitacao = 2;

SELECT usuario.idUsuario AS Usuarios,
nome AS NOME,
CASE
...
IFNULL(cpf, 'Não Informado') AS CPF,
IFNULL(dtNascimento, 'Não Informado') AS DataNascimento,
IFNULL(validade, 'Sem Validade') AS Validade
FROM Habilitacao JOIN Usuario
ON idUsuario = fkUsuario;
...
SELECT idfarmaceutico AS Identificador,
nome AS NOME,
cpf AS CPF,
email AS Email
FROM Farmaceutico;

SELECT Farmaceutico.idFarmaceutico AS Identificador,
Farmaceutico.nome AS Nome,
Farmaceutico.cpf AS CPF,
Farmaceutico.email AS Email,
Farmacia.nome AS Farmacias,
Farmacia.contato AS Telefone,
Farmacia.cnpj AS CNPJ,
Endereco.endereco AS Endereços
FROM Farmaceutico JOIN Farmacia 
ON idFarmacia = fkFarmacia
JOIN Endereco
ON idEndereco = fkEndereco;

UPDATE Farmaceutico JOIN Farmacia ON idFarmacia = fkFarmacia 
SET Farmacia.cnpj = NULL, Farmaceutico.email = NULL 
WHERE Farmaceutico.idFarmaceutico AND Farmacia.idFarmacia IN (1,3,5);


SELECT idFarmaceutico AS Farmaceuticos,
CASE
WHEN Farmaceutico.nome = 'Gustavo' THEN 'Giovany'
WHEN Farmaceutico.nome = 'Rubia' THEN 'Rainy'
WHEN Farmaceutico.nome = 'Luiz' THEN 'Luiza'
WHEN Farmaceutico.nome = 'Gregorio' THEN 'Gregorio Laceda'
ELSE 'Gabriel Miranda'
END AS NOME,
cpf AS CPF,
CASE
WHEN email = 'gustavo@email.com' THEN 'giovany@email.com'
WHEN email = 'luiz@email.com' THEN 'luiza@email.com'
WHEN email = 'felipe@email.com' THEN 'gabrielmiranda@email.com'
WHEN email = 'rubia@email.com' THEN 'rainy@email.com'
END AS Email_Farmaceuticos,
CASE 
WHEN Farmacia.nome = 'Rejuvence' THEN 'Rleste'
WHEN Farmacia.nome = 'SP Drogaria' THEN 'Droga Mil'
WHEN Farmacia.nome = 'Drogazil' THEN 'Farma Feliz'
WHEN Farmacia.nome = 'Drogabras' THEN ''
WHEN Farmacia.nome = 'Longa Vida' THEN 'Viva +'
END AS Farmacias,
IFNULL(Farmacia.cnpj, 'Não Informado') AS CNPJ,
Endereco.endereco AS Endereços,
Farmacia.contato AS Contato
FROM Farmaceutico JOIN Farmacia ON idFarmacia = fkFarmacia JOIN Endereco ON idEndereco = fkEndereco;


SELECT idFarmaceutico AS Idenficador,
nome AS NOME_Farmnaceuticos,
cpf AS CPF_Farmaceuticos,
IFNULL(email, 'Não Infromado') AS EMAIL_Farmaceuticos
FROM Farmaceutico;
